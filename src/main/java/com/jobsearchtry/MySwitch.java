...
public class MySwitch extends CompoundButton {
	...
	private final boolean mTextOnThumb;
	...
	private final Drawable mThumbDrawableOff;
	private final Drawable mThumbDrawableOn;
	private final Drawable mTrackDrawable;
	private final int mThumbTextPadding;
	private final int mTrackTextPadding;
	...
	public void setOn() {
		mThumbDrawable = mThumbDrawableOn;
		invalidate();
	}

	public void setOff() {
		mThumbDrawable = mThumbDrawableOff;
		invalidate();
	}
	...
	public MySwitch(Context context, AttributeSet attrs, int defStyle) {
		...
		mThumbDrawableOn = a.getDrawable(R.styleable.MySwitch_thumb);
		mThumbDrawableOff = a.getDrawable(R.styleable.MySwitch_thumb_off);
		mTrackDrawable = a.getDrawable(R.styleable.MySwitch_track1);
		...
		mThumbTextPadding = a.getDimensionPixelSize(
				R.styleable.MySwitch_thumbTextPadding1, 0);
		mTrackTextPadding = a.getDimensionPixelSize(
				R.styleable.MySwitch_trackTextPadding1, 0);
		mSwitchMinWidth = a.getDimensionPixelSize(
				R.styleable.MySwitch_switchMinWidth1, 0);
		mSwitchMinHeight = a.getDimensionPixelSize(
				R.styleable.MySwitch_switchMinHeight1, 0);
		mSwitchPadding = a.getDimensionPixelSize(
				R.styleable.MySwitch_switchPadding1, 0);

		mTrackDrawable.getPadding(mTrackPaddingRect);
		mThumbDrawable.getPadding(mThPad);
		mThumbDrawableOff.getPadding(mThPad);
		mThumbDrawableOn.getPadding(mThPad);
			...
			if (mMaskDrawable == null) {
				e = new IllegalArgumentException(
						a.getPositionDescription()
								+ " if left/right background is given, then a mask has to be " +
								"there");
			}
		}
		if ((mLeftBackground != null) ^ (mRightBackground != null)) {
			if (mMaskDrawable == null) {
				e = new IllegalArgumentException(
						a.getPositionDescription()
								+ " left and right background both should be there. only one is " +
								"not allowed ");
			}
		}
		...
		if ((mTextOnThumb) && (mPushStyle)) {
			e = new IllegalArgumentException(
					a.getPositionDescription()
							+ " Text On Thumb and Push Stype are mutually exclusive. Only one can " +
							"be present ");
		}
	 ...
	 */
	private void setSwitchTextAppearance(Context context, int resid) {
	 ...
	 * you specified.
	 */
	private void setSwitchTypeface(Typeface tf, int style) {
		if (style > 0) {
			if (tf == null) {
	 ...
	 *
	 * @param fixed if true, sets the switch to fixed mode. if false, sets the
	 *              switch to switched mode.
	 */
	private void fixate(boolean fixed) {
		this.fixed = fixed;
	}
	...
	private Layout makeLayout(CharSequence text) {
		return new StaticLayout(text, mTextPaint,
				(int) (float) Math.ceil(Layout.getDesiredWidth(text,
	...
	private boolean hitThumb(float x, float y) {
		...
		if (mSwitchHeight > 150) {
			...
			Log.d(TAG,
					"returning "
							+ (x > thumbLeft && x < thumbRight && y > thumbTop && y <
							thumbBottom));
			return x > thumbLeft && x < thumbRight && y > thumbTop
					&& y < thumbBottom;
		} else {
	...
	public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		mThumbWidth = Math.max(mThumbWidth, mThumbDrawableOff.getIntrinsicWidth());
		mThumbWidth = Math.max(mThumbWidth, mThumbDrawableOn.getIntrinsicWidth());
		if (!mTextOnThumb) {
	...
	protected void onLayout(boolean changed, int left, int top, int right,
		...
		if (mMaskDrawable != null) {
			...
			mMaskDrawable.setBounds(mSwitchLeft, mSwitchTop, mSwitchRight,
					mSwitchBottom);
			// Log.d(TAG,"bitmap width="+tempBitmap.getWidth()+" bitmap.height="+tempBitmap
			// .getHeight());
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mOrientation == VERTICAL) {
			...
			int thumbBoundT = switchInnerTop + this.getThumbScrollRange()
					- mThumbExtraMovement;
			...
			if (mPushStyle) {
				...
				thumbBoundT = switchInnerTop + this.getThumbScrollRange()
						- mThumbExtraMovement;
			...
			mThumbDrawable.setBounds(mSwitchLeft, thumbBoundT, mSwitchRight,
					thumbBoundB);
			mThumbDrawableOff.setBounds(mSwitchLeft, thumbBoundT, mSwitchRight,
					thumbBoundB);
			mThumbDrawableOn.setBounds(mSwitchLeft, thumbBoundT, mSwitchRight,
					thumbBoundB);
		...
		if (mOrientation == HORIZONTAL) {
			...
			mThumbDrawable.setBounds(thumbBoundL, mSwitchTop, thumbBoundR,
					mSwitchBottom);
			mThumbDrawableOff.setBounds(thumbBoundL, mSwitchTop, thumbBoundR,
					mSwitchBottom);
			mThumbDrawableOn.setBounds(thumbBoundL, mSwitchTop, thumbBoundR,
					mSwitchBottom);
			mThumbDrawable.draw(canvas);
